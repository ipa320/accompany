...
public class ActionPossibilityWidget extends Button implements View.OnClickListener {
	...
	protected boolean removedMark=false;
	...
	protected final int CHANGINGPOSITION=5;
	...
	public ActionPossibilityWidget(Context context,int apid,String Name,double lik,String frase,
			...
			AccompanyActivity a,int precondid,boolean UorR,MediaPlayer media) {
		super(context);
	...
	public void onClick(View v)
	{
		...
		else
		{
			if (state==WAITCONF)
			{
				...
				runAnimX.setDuration(500);runAnimX.setStartDelay(100);
				runAnimY.setDuration(500);runAnimY.setStartDelay(100);
				runAnimH.setDuration(500);runAnimH.setStartDelay(100);
				runAnimW.setDuration(500);runAnimW.setStartDelay(100);
	...
	public void resetMe()
	{
		...
		if (state!=NORMAL)
		{
			if (removedMark)
			{
				...
				act.removeFromList(this);
				act.recomputePositions();
			}
			if (options!=null)
				for (int i=0;i<options.size();i++)
	...
	public void showOptions()
	{
	  ...
	  public void changePosition(int pos_x,int pos_y)
	  {
		  if (state!=NORMAL)
		  {
			  my_X=pos_x;
			  my_Y=pos_y;
			  AbsoluteLayout.LayoutParams p= (AbsoluteLayout.LayoutParams) my_blue_circle.getLayoutParams();
			  p.x=pos_x-20;
			  p.y=pos_y-20;
			  my_blue_circle.setLayoutParams(p);
		  }
		  else
		  {
			  state=CHANGINGPOSITION;
			  ActionPossibilityWidget.this.setClickable(false);
			  ObjectAnimator animMoveX = ObjectAnimator.ofFloat(ActionPossibilityWidget.this,"x",my_X,pos_x );
			  ObjectAnimator animMoveY = ObjectAnimator.ofFloat(ActionPossibilityWidget.this,"y",my_Y,pos_y );
			  animMoveX.setDuration(500);
		      animMoveY.setDuration(500);
			  AnimatorSet animMove= new AnimatorSet();
			  animMove.playTogether(animMoveX,animMoveY);
			  animMove.addListener(new AnimatorListener() {
				@Override
				public void onAnimationStart(Animator animation) {
				}
				@Override
