...
public class UserView extends AccompanyActivity{
	...
	protected AbsoluteLayout container_layout;
	...
	protected double angolo_corrente;
	protected double ultimo_raggio;
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		container_layout=(AbsoluteLayout)this.findViewById(R.id.user_layout_container);
		...
		vto=this.container_layout.getViewTreeObserver();
		pdl=new OnPreDrawListener(){
			...
			public boolean onPreDraw() {
				...
				int ww= container_layout.getMeasuredWidth()-scarto*2;
				int hh= container_layout.getMeasuredHeight()-scarto*2;
				int size=ww;
				if (size>hh) size=hh;
		...
		if (working) 
		{
			showRobotExecutingCommandView();
			working=false;
		}
	}
	
	public void shouldshowRobotExecutingCommandView(){
		working=true;
	}
	...
	{
		//regenerating the layout for aps
		removeAllLabels();
		my_layout=new AbsoluteLayout(this);
		//my_layout.setId(id)
		@SuppressWarnings("deprecation")
		AbsoluteLayout.LayoutParams parss= new AbsoluteLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT, 0, 0);
		my_layout.setLayoutParams(parss);
        ...
        try{
            ...
            for(int i=0;i<jArray.length();i++){
                ...
                ActionPossibilities.add(new ActionPossibilityWidget(getApplicationContext(),
                	...
                	my_layout,this,
                	json_data.getInt("precond_id"),true,mp));  //precond id is the sensor id to be activated to run the AP
            }
	...
	public void showLabels() 
	{
		...
		for (int i=0;i<ActionPossibilities.size();i++)
		{
			...
			angolo_corrente=angolo;
			ultimo_raggio=butt_radius;
	...
	public void removeAllLabels() {
		...
		container_layout.removeView(my_layout);
	}
	...
	public void recomputePositions()
	{
		...
		for (int i=0;i<ActionPossibilities.size();i++)
		{
			...
			x1=x-((int)(ActionPossibilities.get(i).getMyWidth()/2));
			y1=y-((int)(ActionPossibilities.get(i).getMyWidth()/2));
			ActionPossibilities.get(i).changePosition(x1, y1);
			Log.i(TAG,"Adding ap to layout: "+ActionPossibilities.get(i).getName());
			butt_radius=ActionPossibilities.get(i).getMyWidth()/2;
			if (i==ActionPossibilities.size()-1)
				next_r=ActionPossibilities.get(0).getMyWidth()/2;
	...
	public void addAp(int id, String last_r)
	{
	        	...
	        	if (idf==id)
	        	{
		            apw = new ActionPossibilityWidget(getApplicationContext(),
		              	json_data.getInt("apId"),
		               	...
		               	json_data.getString("phraseal_feedback"),
		               	my_layout,this,
		               	json_data.getInt("precond_id"),true,mp);
	        	}
	        }
	   }
	   catch(JSONException e){
	   ...
	   if (apw!=null)
	   {
		   ...
		   double raggio=circle_size/2-10;//prima senza -10
		   int margine=5;
		   angolo_corrente=angolo_corrente+(2*Math.asin((ultimo_raggio + margine+ apw.getMyWidth()/2)/(2*raggio)));
		   int x= circle_x+(int)(raggio*Math.cos(angolo_corrente));
		   int y= circle_y+(int)(raggio*Math.sin(angolo_corrente));
		   int pos_x = (int)(x-apw.getMyWidth()/2);
		   int pos_y = (int)(y-apw.getMyWidth()/2);
