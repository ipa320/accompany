...
public class ActionsListView extends Activity{
	...
	protected ActionButton lastApClicked;
	...
	//Switching to robot view (on right banner clicks), the robot view activity is launched:
		...
		public void showOptionsPd(LinearLayout contentView,final ActionButton ab)
		{
			if(optionPd!=null) optionPd.dismiss();
			
			optionPd= new PopupWindow(contentView,LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT);
			optionPd.setOutsideTouchable(true);
			optionPd.setOnDismissListener(new PopupWindow.OnDismissListener() {
				@Override
				public void onDismiss() {
					ab.reset();
				}
			});
			optionPd.setBackgroundDrawable(getResources().getDrawable(R.drawable.transparent));
			optionPd.showAsDropDown(ab, ab.getWidth()-10, ab.getHeight()-5);
		}
		...
		public void sendOptionsRequest(int idf, ActionButton ab)
		{
			lastApClicked=ab;
	    	myApp.requestOptions(idf);
		}
