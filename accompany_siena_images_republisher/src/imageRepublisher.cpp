...
int count=0;
int width,height;  //desired output image dimensions
...
void image_callback(const sensor_msgs::ImageConstPtr& msg)
{
        ...
        try
        {
                ...
                if (count%sampling_rate==0)
                {
                        ...
                        cv::resize(last_img, tmp_img, size);
                        if (head_position>(-(MAX_HEAD_POS-MIN_HEAD_POS)/2))//<
                        {
                            if (cob_version==COB_3_6)
                            {
                            		cv::Mat tmp_img_2 = cv::Mat::zeros(size, tmp_img.type());
                                    //printf("+++ rotating, head pos: %f\n",head_position);

                            		for (int v=0; v<tmp_img.rows; ++v)
                            			for (int u=0; u<tmp_img.cols; ++u)
                            				tmp_img_2.at<cv::Vec3b>(height-1-v,width-1-u) = tmp_img.at<cv::Vec3b>(v,u);
...
int main(int argc, char** argv)
{
        ...
        std::cout << "Downsizing images to "<<width<<"x"<<height<<"\n";
