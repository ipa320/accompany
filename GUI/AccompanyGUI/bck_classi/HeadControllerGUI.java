...
import pr2_controllers_msgs.JointTrajectoryActionGoal;
...
public class HeadControllerGUI implements NodeMain{
	...
	public void onStart(ConnectedNode cn) {
		...
		/*s= cn.newSubscriber("head_controller/state",pr2_controllers_msgs.JointTrajectoryControllerState._TYPE);
...
/*s= cn.newSubscriber("head_controller/joint_trajectory_action/goal",pr2_controllers_msgs.JointTrajectoryActionGoal._TYPE);
		s.addMessageListener(new MessageListener<pr2_controllers_msgs.JointTrajectoryActionGoal>() {
		      @Override
		      public void onNewMessage(final pr2_controllers_msgs.JointTrajectoryActionGoal message) {
		        double[] a =message.getGoal().getTrajectory().getPoints().get(0).getPositions();
		        head_pos=a[0];
		        if (calendar.getTimeInMillis()-last_publish>3000)
		        	desired_head_pos=head_pos;
		      }
	...
	public void bringCameraToFront()
	{
		...
		desired_head_pos=0;//-Math.PI;
		pr2_msgs.JointTrajectoryActionGoal msg= p.newMessage();
		msg.getJointNames().add("head_axis_joint");
		JointTrajectoryPoint pp=mf.newFromType(JointTrajectoryPoint._TYPE);
		double a[]= new double[1];
		a[0]=desired_head_pos;
