...
public class TorsoControllerGUI implements NodeMain{
	...
	public final double[] max_torso_pos={0.10,0.0,0.1499};
	public final double[] min_torso_pos={-0.10,0.0,-0.1499};
	...
	public void onStart(ConnectedNode cn) {
		...
		/*s= cn.newSubscriber("torso_controller/state",pr2_controllers_msgs.JointTrajectoryControllerState._TYPE);
		s.addMessageListener(new MessageListener<pr2_controllers_msgs.JointTrajectoryControllerState>() {
		      ...
		      public void onNewMessage(final pr2_controllers_msgs.JointTrajectoryControllerState message) {
		        torso_pos=message.getActual().getPositions();
		        if (calendar.getTimeInMillis()-last_publish>3000)
		        	desired_torso_pos=torso_pos;
		      }
		    });*/
		my_seq_count=0;
	...
	public void publish(double difference)
	{
		...
		//try{
			 ...
			 /*if (sense)
			 {
				 desired_torso_pos[0]+=difference;
				 desired_torso_pos[2]+=difference*(0.1499/0.10);
			 }
			 else
			 {
				 desired_torso_pos[0]-=difference;
			 ...
			 desired_torso_pos[0]+=difference;
			 desired_torso_pos[2]+=difference*(0.1499/0.10);
			 if (desired_torso_pos[0]<min_torso_pos[0])
				 desired_torso_pos[0]=min_torso_pos[0];
			 if(desired_torso_pos[2]<min_torso_pos[2])
				 desired_torso_pos[2]=min_torso_pos[2];
			 if (desired_torso_pos[0]>max_torso_pos[0])
				 desired_torso_pos[0]=max_torso_pos[0];
			 if(desired_torso_pos[2]>max_torso_pos[2])
				 desired_torso_pos[2]=max_torso_pos[2];
			 a[0]=desired_torso_pos[0];
			 a[1]=torso_pos[1];
		...
		/*}catch(Exception e)
		{
			Log.e("AccompanyGUI-Torso controller","Error: cannot send torso command message!");
		}*/
	    p.publish(msg);
