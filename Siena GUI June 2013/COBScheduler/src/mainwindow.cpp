...
QSqlDatabase db8;                                       //Siena's connection for monitoring the robot state    MARCOB
bool dbOpen;
...
QString dname;
 ...
 {
    sleep(0.3); //to avoid double command from Siena GUI turning function
    ...
    QString sleepString = "S1-sleep";                                                //MARCOB
    if (currentlyExecutingSequence.compare(sleepString)!=0) 
    {
   	QSqlQuery setStateQuery(db8);

    	setStateQuery.clear();

    	setStateQuery.prepare("UPDATE Sensors SET value = '0' WHERE sensorId = '999' ");
	
    	if (!setStateQuery.exec())
    	{
		qDebug() << "error updating robot status!";
    	}
    }
...
int MainWindow::sendScriptServerMsg()
{
    ...
    if (client->getState() != actionlib::SimpleClientGoalState::SUCCEEDED)
    {
       ...
       //returnResult = SCRIPTSERVER_EXECUTION_FAILURE;                                 //MARCOB
    }
...
void MainWindow::runSequence(QString sequenceName, int priority, QString CanInterrupt, int row)
{
    ...
    QString sleep = "S1-sleep";
    //qDebug() <<"-------------------------------------------> -" + sequenceName + "-";	
    if (sequenceName.compare(sleep)!=0)                                                         //MARCOB
    {
	qDebug() << "not sleep!";
	QSqlQuery setStateQuery(db8);
	...
	setStateQuery.prepare("UPDATE Sensors SET value = '1' WHERE sensorId = '999' ");
	...
	setStateQuery.clear();
        ...
        setStateQuery.prepare("INSERT INTO RobotActionsHistory (timestamp,actionName) VALUES (NOW(),:action)");

        setStateQuery.bindValue(":action", sequenceName);

	if (!setStateQuery.exec())
        {
		qDebug() << "error updating robot actions history!";
        }
    }
    else
    {
...
void MainWindow::checkExecutionResult()
{
   ...
   //qDebug() <<"-------------------------------------------> check start executionResult";	
...
bool MainWindow::openAllDatabaseConnections(QString host, QString user, QString pw)
{
    ...
    if (!openDatabase("db8",host,user,pw, db8))
    {
        return false;
    }
