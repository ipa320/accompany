...
struct WorldPoint {
    double x, y, z;
    WorldPoint(double x=0, double y=0, double z=0) : x(x),y(y),z(z) {}
    WorldPoint &operator+=(const WorldPoint &p) { x+=p.x; y+=p.y; z+=p.z; return *this; }
    WorldPoint &operator/=(const double &d) { x/=d; y/=d; z/=d; return *this; }
};
...
class CamCalib : public XmlPackable {
  public:
    ...
    FLOAT stdev1, pixelVar1, pv2_1, lpv2_1;
    FLOAT stdev2, pixelVar2, pv2_2, lpv2_2;
    FLOAT stdev3, pixelVar3, pv2_3, lpv2_3;
    FLOAT scale;
    ...
    void genFleuretTemplate(const WorldPoint &pt, std::vector<CvPoint> &points) const;

    // void img2vec(const IplImage *img, vnl_vector<FLOAT> &v);
    // IplImage *vec2img(const vnl_vector<FLOAT> &v) const;
    void computeBGProb(const vnl_vector<FLOAT> &img, const vnl_vector<FLOAT> &bg,
        vnl_vector<FLOAT> &sumPixelProb, FLOAT &total);
    void computeBGProbDiff(const vnl_vector<FLOAT> &img, const vnl_vector<FLOAT> &bg,
        vnl_vector<FLOAT> &sumPixelProb, FLOAT &total);
...
void plotTemplate(IplImage *img, const std::vector<CvPoint> &points, const CvScalar &colour, unsigned lw=1);
...
void genScanLocations(const std::vector<WorldPoint> &prior, double scanRes, std::vector<WorldPoint> &sl);
