...
CvScalar BLUE = CV_RGB(0,0,255), GREEN = CV_RGB(0,255,0), RED = CV_RGB(255,0,0),
    CLR = CV_RGB(0,255,0);
...
void showScaledImg(const char *win, IplImage *img)
{
  IplImage *scaled = cvCreateImage(
      cvSize(img->width * scale, img->height * scale), IPL_DEPTH_8U, 3);
...
void refresh()
{
  ...
  for (unsigned j = 0; j != locations.size(); ++j)
  {
    ...
    cvLine(tmp, locations[j],
        locations[j]
            + cvPoint(2 * radius * cos(locations[j].angle),
                2 * radius * sin(locations[j].angle)), RED, 1);
