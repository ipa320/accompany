...
void saveCameraParams(const string& filename, Size imageSize, Size boardSize,
    float squareSize, float aspectRatio, int flags, const Mat& cameraMatrix,
    const Mat& distCoeffs, const vector<Mat>& rvecs, const vector<Mat>& tvecs,
...
bool runAndSave(const string& outputFilename,
    const vector<vector<Point2f> >& imagePoints, Size imageSize, Size boardSize,
    Pattern patternType, float squareSize, float aspectRatio, int flags,
    Mat& cameraMatrix, Mat& distCoeffs, bool writeExtrinsics, bool writePoints)
{
  ...
  bool ok = runCalibration(imagePoints, imageSize, boardSize, patternType,
      squareSize, aspectRatio, flags, cameraMatrix, distCoeffs, rvecs, tvecs,
  ...
  if (ok)
    saveCameraParams(outputFilename, imageSize, boardSize, squareSize,
        ...
        writePoints ? imagePoints : vector<vector<Point2f> >(), totalAvgErr);
  return ok;
}
...
int main(int argc, char** argv)
{
  ...
  optionsDescription.add_options()("height,h",
      po::value<int>(&height)->required(),
      "the number of inner corners per one of board dimension\n")("width,w",
      po::value<int>(&width)->required(),
      "the number of inner corners per another board dimension\n")("output,o",
      po::value<string>(&outputFilename)->required(),
      "the output filename for intrinsic parameter\n")("input,i",
      po::value<string>(&inputFilename)->required(),
      "the text file with a list of the images of the board\n")("undist,u",
      po::value<bool>(&undistortImage)->default_value(false),
      "show undistorted images after calibration\n")("display,d",
      po::value<unsigned int>(&display)->default_value(0),
      "display time of calibrated chessboard patterns (in mini-seconds), default 0 means no display\n");
  try
  {
    ...
    if (!view.data)
    {
      if (imagePoints.size() > 0)
        runAndSave(outputFilename, imagePoints, imageSize, boardSize, pattern,
            squareSize, aspectRatio, flags, cameraMatrix, distCoeffs,
