 ...
 *
 **/
void CamCalib::genFleuretTemplate(const WorldPoint &pt, vector<CvPoint> &points) const
{
	double x = pt.x, y = pt.y, ratio = personHeight / wg;

	CvPoint
	midGround = project(WorldPoint(x,y)),
	midTop = project(WorldPoint(x,y,personHeight));
...
void CamCalib::computeBGProb(const vnl_vector<FLOAT> &img, const vnl_vector<FLOAT> &bg,
		vnl_vector<FLOAT> &sumPixelProb, FLOAT &total)
{
	// Compute p(img|bg)
	vnl_vector<FLOAT>
	diff = img-bg;
...
void CamCalib::computeBGProbDiff(const vnl_vector<FLOAT> &img, const vnl_vector<FLOAT> &bg,
		vnl_vector<FLOAT> &sumPixelProb, FLOAT &total)
{
	// Compute p(img|bg)
	vnl_vector<FLOAT>
...
void CamCalib::computeDiff(const vnl_vector<FLOAT> &img, const vnl_vector<FLOAT> &bg,
		vnl_vector<FLOAT> &diffImg)
{
	// Compute p(img|bg)
	vnl_vector<FLOAT>
	diff = img-bg;
...
bool inside(const WorldPoint &p, const vector<WorldPoint> &prior)
{
	...
	for (unsigned i=1; i<prior.size(); ++i) {
		if ((p.y >= prior[i-1].y && p.y < prior[i].y) || (p.y >= prior[i].y && p.y < prior[i-1].y) ) {
			double
			deltaX = prior[i].x - prior[i-1].x,
			deltaY = prior[i].y - prior[i-1].y;
			if (fabs(deltaX)>fabs(deltaY)) {
				double
				a = deltaY/deltaX,
				b = prior[i].y - a * prior[i].x,
				x = (p.y - b) / a;
				if (x < p.x)
					nLeft++;
				else             // on the line is out
				nRight++;
			}
			else {
				double
				a = deltaX/deltaY,
				b = prior[i].x - a * prior[i].y,
				x = a*p.y + b;
				if (x < p.x)
					nLeft++;
				else             // on the line is out
 ...
 *
 **/
struct GridElt { unsigned row, col; GridElt(unsigned r, unsigned c) : row(r),col(c) {}};
...
{
	unsigned r = gridLocations[elt].row + shiftRow,
			c = gridLocations[elt].col + shiftCol;
...
void genScanLocations(const vector<WorldPoint> &prior, double scanRes, vector<WorldPoint> &sl)
{
	...
	for (unsigned i=0; i!=prior.size(); ++i) {
		if (minX > prior[i].x) minX = prior[i].x;
		if (maxX < prior[i].x) maxX = prior[i].x;
		if (minY > prior[i].y) minY = prior[i].y;
		if (maxY < prior[i].y) maxY = prior[i].y;
	}

	sl.clear();
	unsigned r=0, c=0;
	gridElts = vector< vector<unsigned> >((maxY - minY)/scanRes+1, vector<unsigned>((maxX-minX)/scanRes+1,(unsigned)-1));
