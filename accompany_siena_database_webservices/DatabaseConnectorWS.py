...
class setemphasis:
	def GET(self):
		paras = web.input(speed='0.0')
		value = web.websafe(paras.speed)
		...
		db= MySQLdb.connect(HOST,U,P,DATABASE_NAME)
		cur= db.cursor()
		sql = ('UPDATE Accompany.Sensors SET value=\'%s\',lastTimeActive=NOW(),lastUpdate=NOW(),lastActiveValue=\'%s\' WHERE sensorId=\'1000\';'%(value,value))		
		cur.execute(sql);
		
		print "aaa"
		cur=db.cursor()
		sql=('INSERT INTO SqueezeHistory (timestamp,value) VALUES (NOW(),\'%s\'); '%value)
		cur.execute(sql)
		print "OK"
		return "ok"
