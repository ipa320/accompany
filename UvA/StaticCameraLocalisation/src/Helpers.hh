...
bool masksOverlap(const std::vector<scanline_t> &m1, const std::vector<scanline_t> &m2);
...
void listImages(const char *list, std::vector<std::string> &imgs,std::vector<double> &tstamps);
...
void plotScanLines(IplImage *img, const std::vector<scanline_t> &mask, const CvScalar &colour, double alpha);
