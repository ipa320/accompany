...
#					handle_base=sss.move("base",[rx, ry, theta], blocking=False)
					if self.user_position.point.x > -1 and self.user_position.point.y < -2.2:
...
class SM_walkTogetherToKitchen(smach.StateMachine):
	def __init__(self):
...
if __name__=='__main__':
	try:
		rospy.init_node('walkTogetherToKitchen')
		sm = SM_walkTogetherToKitchen()
