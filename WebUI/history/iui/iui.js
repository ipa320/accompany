...
window.iui =
{
	...
	insertPages: function(frag)
	{
		...
		for (var i = 0; i < nodes.length; ++i)
		{
			...
			if (child.nodeType == 1)
			{
				...
				if (child.getAttribute("selected") == "selected" || !targetPage)
					targetPage = child;
	...
	*/
	getSelectedPage: function()
	{
		...
		{
			if (child.nodeType == 1 && child.getAttribute("selected") == "selected")
				return child;
		}	 
	},
...
{
	currentDialog = page;
	page.setAttribute("selected", "selected");
...
function slide1(fromPage, toPage, backwards, axis, cb)
{
	...
	scrollTo(0, 1);
	toPage.setAttribute("selected", "selected");
	var percent = 100;
	slide();
