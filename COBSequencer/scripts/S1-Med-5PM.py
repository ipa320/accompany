...
#--------------------------------------------------------------------------------
class S1-Med-5PM(script):

  def Initialize(self):

    if not self.sss.parse:
       rospy.loginfo(" *********** This is the S1-Med-5PM script ************** ")
...
#--------------------------------------------------------------------------------
  def Run(self):
    ...
    if not self.sss.parse:
       rospy.loginfo("move torso on ::0::Care-O-Bot 3.2 to the forward position and to the right and wait for completion")
    ...
    if not self.sss.parse:
       rospy.loginfo("Turn light on ::0::Care-O-Bot 3.2 to  white")
    ...
    if not self.sss.parse:
       rospy.loginfo("::0::Care-O-Bot 3.2 says 'It's time for your medicine'")

    if (overallresult):
       self.sss.say(["It's time for your medicine"])
    else:
      if not self.sss.parse:
         rospy.loginfo("Action not executed as rule is invalid!")
    ...
    if not self.sss.parse:
       rospy.loginfo("Wait for 5 seconds on ::0::Care-O-Bot 3.2")
    ...
    if not self.sss.parse:
       rospy.loginfo("::0::Care-O-Bot 3.2 GUI,doNothing,returnToBase,doNothing")
    ...
    if not self.sss.parse:
       rospy.loginfo("SET ::2::5PM-Medicine-Due TO  false")

    if (overallresult):
       if not self.sss.parse:
          cursorSequence = self.conn.cursor()
          sqlSequence = "UPDATE ActionGoals SET value = 2 WHERE goalId = 0"
    ...
    if not self.sss.parse:
       rospy.loginfo("SET ::3::5PM-Medicine-Confirm TO  true")

    if (overallresult):
       if not self.sss.parse:
          cursorSequence = self.conn.cursor()
          sqlSequence = "UPDATE ActionGoals SET value = 3 WHERE goalId = 1"
          cursorSequence.execute(sqlSequence)
    else:
      if not self.sss.parse:
         rospy.loginfo("Action not executed as rule is invalid!")
...
def displayGUI(self):
   ...
   if not self.sss.parse:
      ...
      while awaitingUserResponse:
         ...
         sqlSequence = "SELECT * FROM  userInterfaceGUI WHERE name = 'S1-Med-5PM'"
         cursorSequence.execute(sqlSequence)      # get the sequence from the sql query
         sequenceRows = cursorSequence.fetchall()
         ...
         try:
          ...
          for sequenceRow in sequenceRows:      # for each row execute the rule to see it is valid 
            ...
            if (resultFromUser == 1):
                proc = subprocess.Popen("/home/joe/QTProjects/COBSequencer/scripts/doNothing.py",shell=True, stderr=subprocess.PIPE)
            ...
            if (resultFromUser == 2):
                proc = subprocess.Popen("/home/joe/QTProjects/COBSequencer/scripts/returnToBase.py",shell=True, stderr=subprocess.PIPE)
                return_code = proc.wait()
                for line in proc.stderr:
                  if (line.rstrip() == "Failure"):
                     overallresult = False
