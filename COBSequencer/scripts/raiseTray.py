...
#--------------------------------------------------------------------------------
class raiseTray(script):

  def Initialize(self):

    if not self.sss.parse:
       rospy.loginfo(" *********** This is the raiseTray script ************** ")
...
#--------------------------------------------------------------------------------
  def Run(self):
    ...
    if not self.sss.parse:
       rospy.loginfo("SET ::0::trayIsRaised TO  true")

    if (overallresult):
       if not self.sss.parse:
          cursorSequence = self.conn.cursor()
          sqlSequence = "UPDATE ActionGoals SET value = 0 WHERE goalId = 1"
    ...
    if not self.sss.parse:
       rospy.loginfo("SET ::1::trayIsLowered TO  false")

    if (overallresult):
       if not self.sss.parse:
          cursorSequence = self.conn.cursor()
          sqlSequence = "UPDATE ActionGoals SET value = 1 WHERE goalId = 0"
          cursorSequence.execute(sqlSequence)
    else:
      if not self.sss.parse:
         rospy.loginfo("Action not executed as rule is invalid!")
