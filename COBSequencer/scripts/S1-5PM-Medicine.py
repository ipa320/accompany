...
#--------------------------------------------------------------------------------
class S1-5PM-Medicine(script):

  def Initialize(self):

    if not self.sss.parse:
       rospy.loginfo(" *********** This is the S1-5PM-Medicine script ************** ")
...
#--------------------------------------------------------------------------------
  def Run(self):
    ...
    if not self.sss.parse:
       rospy.loginfo("::0::Care-O-Bot 3.2 says 'You need to take your medicine now'")

    if (overallresult):
       self.sss.say(["You need to take your medicine now"])
    else:
      if not self.sss.parse:
         rospy.loginfo("Action not executed as rule is invalid!")
    ...
    if not self.sss.parse:
       rospy.loginfo("Turn light on ::0::Care-O-Bot 3.2 to  blue")
    ...
    if not self.sss.parse:
       rospy.loginfo("::0::Care-O-Bot 3.2 GUI,doNothing,doNothing,doNothing")
...
def displayGUI(self):
   ...
   if not self.sss.parse:
      ...
      while awaitingUserResponse:
         ...
         sqlSequence = "SELECT * FROM  userInterfaceGUI WHERE name = 'S1-5PM-Medicine'"
         cursorSequence.execute(sqlSequence)      # get the sequence from the sql query
         sequenceRows = cursorSequence.fetchall()
